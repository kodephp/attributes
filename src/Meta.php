...
final class Meta implements JsonSerializable, Stringable
{
     ...
     * 声明唯一标识：声明主体（成员名）+ 属性类。
     *
     * 继承合并按此去重，语义是「就近覆盖」而非「父子并存」：子类重写父类成员并在其上
     * 重新声明同一个非可重复属性时，只保留子类的声明（先遍历到者生效）；子类没重新声明
     * 时，父类的声明照常继承。同一声明被重复遍历（接口/特性别名）也走同一条去重。
     * 标识里刻意不含声明类：含了就变成父子两条并存，非可重复属性会在同一次读取里出现两份。
     * 纯数据模式（无 reflector）主体记为 global。
     */
